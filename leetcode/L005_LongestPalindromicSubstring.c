...
char L005_0_access_char_array(char* s, int i) {
    if (i % 2 == 0) return '\0';
    return *(s + i / 2);
}

char* L005_0_longestPalindrome(char* s) {
    int n = strlen(s);
    int ma_n = 2 * n + 1;
    int ma_i = 0;
    int ma_li = 0;
    ...
    for (ma_i = 0; ma_i < ma_n; ma_i ++) {
        ...
        if (ma_i >= ma_mt || (ma_i + *(ma_r + ma_mi) == ma_mt)) {
            l = ma_i;
            r = ma_i;
            while ((l - 1) > -1 && (r + 1) < ma_n && L005_0_access_char_array(s, l - 1) == L005_0_access_char_array(s, r + 1)) {
                l --;
                r ++;
            }
            *(ma_r + ma_i) = (r - l) / 2;
    ...
    *(answer + *(ma_r + ma_an)) = '\0';
    ...
    for (i = l; i <= r; i ++) {
        if (i % 2 == 1)
            *(answer + (j ++)) = L005_0_access_char_array(s, i);
    }
    free(ma_r);
    return answer;
...
int L005() {
    char* s = "cbbd";
    char* a = L005_0_longestPalindrome(s);
    unsigned int i = 0;
    printf("len(a) is %d\r\n", strlen(a));
