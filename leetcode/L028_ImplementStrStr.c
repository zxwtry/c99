...
int* L028_get_next(char* p, int pn) {
    int bi = 0, fi = 0;
...
void L028_print_array(int * nums, int numsSize) {
    int i = 0;
...
int L028_strStr_kmp(char* s, char* p) {
    int sn = strlen(s), pn = strlen(p);
    int * next = L028_get_next(p, pn);
    int si = 0, pi = 0;
...
int L028_strStr_back(char* s, char* p) {
    int i = 0, j = 0;
    char sc = '\0', pc = '\0';
    if (s == NULL || p == NULL)
