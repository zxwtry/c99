...
int L069_mySqrt(int x) {
    return (int) sqrt(x);
}

int L069_cmp2(int sqrt_val, int x) {
...
int L069_mySqrt2(int x) {
    ...
    while (sqrt_min < sqrt_max) {
        ...
        if (L069_cmp2(sqrt_mid, x) <= 0) {
            if (L069_cmp2(sqrt_mid+1, x) > 0)
                return sqrt_mid;
            sqrt_min = sqrt_mid + 1;
        } else {
            sqrt_max = sqrt_mid - 1;
