...
typedef struct L057_Interval si;

struct L057_Interval {
    int start;
    int end;
};

int L057_cmp_real(si* n, int ni, int nn, int val) {
    si *n1 = NULL, *n2 = NULL;
    if (ni % 2 == 0) {
...
int L057_state(si* n, int nn, int val) {
    int i = 0, j = 2  * nn, m = 0, cmp_val = 0;
    while (i < j) {
        m = i + (j - i) / 2;
        cmp_val = L057_cmp_real(n, m, nn, val);
...
si* L057_insert(si* n, int nn, si new_n, int* rn) {
    int i = L057_state(n, nn, new_n.start);
    int j = L057_state(n, nn, new_n.end);
    int k = 0, cut = 0, v = 0;
    int r1 = 0, r2 = 0, ai = 0;
    si* ans = NULL;
    if (i == j) {
