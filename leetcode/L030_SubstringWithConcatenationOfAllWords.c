...
int L030_cmp(char * s1, int i1, char * s2, int i2, int len) {
    int i = 0;
    for (i = 0; i < len; i ++) {
        if (*(s1 + i1 + i) != *(s2 + i2 + i))
            return 0;
...
int L030_contains(char * s1, char ** record_char, int record_size, int len) {
    int i = 0;
    for (i = 0; i < record_size; i ++) {
        if (L030_cmp(s1, 0, *(record_char + i), 0, len))
            return i;
    }
    return -1;
}

int* L030_findSubstring(char* s, char** words, int len2, int* returnSize) {
    ...
    for (i = 1; i < len2; i ++) {
        for (j = 0, sign = 0; ! sign & (j < i); j ++)
            sign |= L030_cmp(*(words + j), 0, *(words + i), 0, wl);
        if (! sign) record_size ++;
    }
    ...
    *(target_int + 0) += 1;
    ...
    for (i = 0; i < wl; i ++) {
        ...
        for (j = i; j <= len1 - wl; j += wl) {
            v = L030_contains(s + j, record_char, record_size, wl);
            if (v != -1) {
                *(record_int + v) += 1;
                count ++;
                while (*(target_int + v) < *(record_int + v)) {
                    vv = L030_contains(s + left, record_char, record_size, wl);
                    *(record_int + vv) -= 1;
                    count --;
                    left += wl;
                }
...
int L030() {
    ...
    *(words + 2) = w2;
    *(words + 3) = w3;
    answer = L030_findSubstring(s, words, len2, &returnSize);
