...
typedef char** T;
typedef struct L015_al sal;
typedef struct L015_al * pal;
...
void L015_al_expand_capacity(pal l) {
...
void L015_al_add_last(pal l, T v) {
    if (l->capacity == l->size) L015_al_expand_capacity(l);
    l->arr[l->size] = v;
    l->size ++;
}

void L015_al_add_to_index(pal l, T v, int index) {
    int i = 0;
...
T* L015_al_convert_to_array_free_l(pal l) {
    T* arr = l->arr;
    free(l);
    return arr;
}

T L015_al_access_by_index(pal l, int index) {
...
int L015_valid(char** save, int i, int j, int n) {
    int k = 0, ii = 0, jj = 0;
    for (k = 0; k < n; k ++) {
...
void L015_solve(pal l, char** save, int row, int n) {
    ...
    if (row == n) {
        ...
        L015_al_add_last(l, temp);
        return;
    }
    for (col = 0; col < n; col ++) {
        if (L015_valid(save, row, col, n)) {
            save[row][col] = 'Q';
            L015_solve(l, save, row+1, n);
            save[row][col] = '.';
        }
    }
}

char*** L015_solveNQueens(int n, int* r) {
    ...
    L015_solve(l, save, 0, n);
    *r = l->size;
    return L015_al_convert_to_array_free_l(l);
}


void L015_solve2(pal l, char** save, int row, int n, int* col_sign, int* lup_sign, int* rup_sign) {
    char** temp = NULL, *temp_row = NULL;
    int i = 0, j = 0, col = 0;
    if (row == n) {
        temp = (char**) malloc(sizeof(char*) * n);;
    ...
    for (col = 0; col < n; col ++) {
        if (!col_sign[col] && !lup_sign[col - row + n - 1] && !rup_sign[row + col]) {
            ...
            save[row][col] = 'Q';
            L015_solve2(l, save, row+1, n, col_sign, lup_sign, rup_sign);
            save[row][col] = '.';
...
char*** L015_solveNQueens2(int n, int* r) {
    char** save = (char**) malloc(sizeof(char*) * n);
    char* save_row = NULL;
    int i = 0, j = 0;
    pal l = (pal) malloc(sizeof(sal));
    ...
    L015_solve2(l, save, 0, n, col_sign, lup_sign, rup_sign);
    *r = l->size;
    free(col_sign);
    free(lup_sign);
    free(rup_sign);
